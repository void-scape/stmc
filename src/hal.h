...
void gpio_configure_pin(GPIO_TypeDef *port, uint32_t pin, GPIO_Mode mode,
                        GPIO_OutputType type, GPIO_Speed speed,
...
void gpio_configure_push_pull_low_speed(GPIO_TypeDef *port, uint32_t pin,
                                        GPIO_Mode mode) {
    gpio_configure_pin(port, pin, mode, GPIO_OUTPUT_TYPE_PUSH_PULL,
                       GPIO_SPEED_LOW, GPIO_PULLUPDOWN_NO_PULLUP_DOWN);
}
...
void clocks_enable(void) {
    ...
    RCC->PLLCFGR = (0b10 << RCC_PLLCFGR_PLLSRC_Pos) |
                   (1 << RCC_PLLCFGR_PLLM_Pos) | (27 << RCC_PLLCFGR_PLLN_Pos) |
                   (0b00 << RCC_PLLCFGR_PLLR_Pos) | RCC_PLLCFGR_PLLREN;
...
void tim_enable_interrupt(TIM_TypeDef *tim, IRQn_Type interrupt,
                          uint32_t period, uint32_t prescalar) {
    // Enable tim clock
    switch ((uint32_t)tim) {
    case (uint32_t)TIM1: {
...
// Enable the LPUART with pins PG7 and PG8 at 115200 baud rate, optionally
// enabling the `LPUART1_IRQn`.
void lpuart_enable(LPUart_Interrupt interrupt, uint32_t baud) {
    ...
    // Select the high speed internal (HSI) oscillator as the clock to LPUART1
    // (16MHz)
    ...
    gpio_configure_pin(GPIOG, 7, GPIO_MODE_ALTERNATE,
    ...
    gpio_configure_pin(GPIOG, 8, GPIO_MODE_ALTERNATE,
                       GPIO_OUTPUT_TYPE_PUSH_PULL, GPIO_SPEED_VERY_HIGH,
                       GPIO_PULLUPDOWN_NO_PULLUP_DOWN);
    ...
    // The order here is very sensitive, and this section gave me a lot of
    // trouble.
    //
    // In the original code, the CR1 is configured with 0xD, which enables RE,
    // TE, and UE, but it does not set the FIFOEN. Without setting the FIFOEN,
    // transmitting data worked but receiving data didn't. Then, it turns, out,
    // you have to set the FIFOEN _before_ enabling the LPUART with UE.
    ...
    default: {
        // NOTE: Don't actually disable, I haven't checked if there is a hander
        // here that does something important!
...
void adc_enable_ext_trig(ADC_TypeDef *adc, ADC_In channel, uint32_t ms,
                         ADC_Inject inject) {
    ...
    // Once the conversion is done, the EOC flag is raised and the IRQ handler
    // for ADC is called.
...
void dac_write(DAC_TypeDef *dac, uint32_t data) { dac->DHR12R1 = data; }
...
void perf_start(void) { _perf_start = DWT->CYCCNT; }
