...
uint16_t melody[] = {659, 494, 523, 587, 523, 494, 440, 440, 523, 659, 587, 523, 494, 523, 587,
                     659, 523, 440, 440, 440, 494, 523, 587, 698, 880, 784, 698, 659, 523, 659,
                     587, 523, 494, 494, 523, 587, 659, 523, 440, 440, 0,   659, 494, 523, 587,
                     523, 494, 440, 440, 523, 659, 587, 523, 494, 523, 587, 659, 523, 440, 440,
                     440, 494, 523, 587, 698, 880, 784, 698, 659, 523, 659, 587, 523, 494, 494,
                     523, 587, 659, 523, 440, 440, 0,   659, 523, 587, 494, 523, 440, 415, 494,
                     0,   659, 523, 587, 494, 523, 659, 880, 831, 0};
uint16_t duration[] = {416, 208, 208, 416, 208, 208, 416, 208, 208, 416, 208, 208, 624, 208, 416,
                       416, 416, 416, 208, 416, 208, 208, 624, 208, 416, 208, 208, 624, 208, 416,
                       208, 208, 416, 208, 208, 416, 416, 416, 416, 416, 416, 416, 208, 208, 416,
                       208, 208, 416, 208, 208, 416, 208, 208, 624, 208, 416, 416, 416, 416, 208,
                       416, 208, 208, 624, 208, 416, 208, 208, 624, 208, 416, 208, 208, 416, 208,
                       208, 416, 416, 416, 416, 416, 416, 833, 833, 833, 833, 833, 833, 833, 416,
                       208, 833, 833, 833, 833, 416, 416, 833, 833, 0};
uint32_t num_notes = 99;
...
void dma1_enable(void) {
    ...
    // return to start after exhuasting the memory
    BIT_SET(DMA1_Channel1->CCR, DMA_CCR_CIRC_Pos);

    // read from the melody data
    DMA1_Channel1->CM0AR = (uint32_t)&melody;
    // write to the TIM2 prescalar
    DMA1_Channel1->CPAR = (uint32_t)&TIM2->PSC;
    // read 2 bytes from memory
    DMA1_Channel1->CCR |= 1 << DMA_CCR_MSIZE_Pos;
    // write 2 bytes to TIM2 prescalar
    DMA1_Channel1->CCR |= 1 << DMA_CCR_PSIZE_Pos;
    // define the number of elements in melody
    DMA1_Channel1->CNDTR = num_notes;

    // TIM3_UP triggers dma1 ch1
...
void dma2_enable(void) {
    ...
    DMA2_Channel1->CCR = 0;
    // read from memory
    BIT_SET(DMA2_Channel1->CCR, DMA_CCR_DIR_Pos);
    // increment memory position after read
    BIT_SET(DMA2_Channel1->CCR, DMA_CCR_MINC_Pos);
    // return to start after exhuasting the memory
    BIT_SET(DMA2_Channel1->CCR, DMA_CCR_CIRC_Pos);

    // read from the duration data
    DMA2_Channel1->CM0AR = (uint32_t)&duration;
    // write to the TIM3 ARR
    DMA2_Channel1->CPAR = (uint32_t)&TIM3->ARR;
    // read 2 bytes from memory
    DMA2_Channel1->CCR |= 1 << DMA_CCR_MSIZE_Pos;
    // write 2 bytes to TIM2 prescalar
    DMA2_Channel1->CCR |= 1 << DMA_CCR_PSIZE_Pos;
    // define the number of elements in duration
    DMA2_Channel1->CNDTR = num_notes;

    // TIM3_UP triggers dma2 ch1
    DMAMUX1_Channel8->CCR = 65;

    // enable dma2 ch1
    BIT_SET(DMA2_Channel1->CCR, DMA_CCR_EN_Pos);
}
...
int main(void) {
    ...
    // Enable timer 2 that will be used to generate the melody by adjusting the prescaler
    TIM2->CR1 |= 1;
    // Enable timer 3 that will be used to keep the melody running for certain durations
    TIM3->CR1 |= 1;
